...
class TestAssistanceAnnotation(object):
    ...
    @staticmethod
    def test_assistance_annotation(annotation_list=None):
        if annotation_list is not None:
            for annotation in annotation_list:
                ...
                if annotation.primary_action_normalized_name is not None:
                    assert len(annotation.primary_action_normalized_name) > 0
                if annotation.modality is not None:
                    assert len(annotation.modality) > 0
                if annotation.primary_action_surface_form is not None:
                    assert len(annotation.primary_action_surface_form) > 0
