...
def get_patient_ids(cohort_endpoint, cql_name):
    ...
    baseurl = cohort_endpoint + "/cohort-service/libraries/" + cql_name + "/patientIds"
...
def cql_bulkextract(cql = None):
    ...
    baseurl = cohort_endpoint + "/cohort-service/libraries"
    resp = requests.get(baseurl, verify=False)
    if resp.status_code == 200:
        ...
        if cql not in lib_list:
            return generate_response(500, {"available libraries": str(lib_list),
    ...
    try:
        ...
        local_file_name_dict = build_new_temp_ndjson_file(cos_urls)
        ...
        return generate_response(200, {"cos_target": target_name,
                                       "cos_bucket": bucket_name,
                                       "number_of_resources": total_resources,
                                       "number_of_patients": len(patient_ids)})
    except Exception as e:

        return generate_response(500, {"message": "Something went wrong in the cql bulk export processing",
