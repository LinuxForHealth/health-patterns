...
def parse_fhir_resource_from_payload(doc: bytes) -> Resource:
    ...
    raises UserError if the provided data is not valid
    """
    ...
    try:
        ...
        raise UserError(message=f"Resource was not valid json: {str(jderr)}") from jderr
    ...
    if "resourceType" in obj and obj["resourceType"] in parsers:
        try:
            ...
            raise UserError(
                message="Resource was not valid", json_details=verr.json()
            ) from verr
        ...
        raise UserError(message=f"The resource type {resource_type} is not supported")
